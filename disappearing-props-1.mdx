---
title: "The Mysterious Case of the Disappearing Props test testing"
...
# The Mysterious Case of the Disappearing Props test testing
...
```jsx
const MysteryComponent = ({ clue }) => <div>{clue ? `Clue: ${clue}` : "No clue found."}</div>;
```
...
const MysteryComponent = ({ clue }) => {
    return <>{clue ? <div>Clue: {clue}</div> : <div className="warning">Warning: Clue is missing!</div>}</>;
};
```
